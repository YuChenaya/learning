...
class Vocab:
    """自定义词汇表类，用于替换torchtext.vocab.Vocab"""
    def __init__(self, counter, min_freq=1):
        self.freqs = counter
        # 保留频率大于等于min_freq的词
        self.itos = ['<unk>']  # 0号位置为未知词
        self.stoi = {'<unk>': 0}
    ...
    def __getitem__(self, token):
        return self.stoi.get(token, 0)  # 默认返回未知词的索引0
...
def load_pretrained_embedding(words, pretrained_vocab, vectors=None):
    """从预训练好的vocab中提取出words对应的词向量
    
    Args:
        words: 词汇列表
        pretrained_vocab: 预训练词汇表
        vectors: 预训练词向量矩阵，如果为None，则返回随机初始化的词向量
    """
    # 如果没有提供预训练词向量，则返回随机初始化的词向量
    if vectors is None:
        embed_dim = 100  # 默认词向量维度
        return torch.randn(len(words), embed_dim) * 0.01
        
    embed = torch.zeros(len(words), vectors.shape[1])  # 初始化为0
